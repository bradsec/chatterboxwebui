...
function getFormData() {
  ...
  // Get form values with safe parsing and validation
  const exaggeration = safeParseFloat(safeGetElementValue('exaggeration'), 0.5);
  const temperature = safeParseFloat(safeGetElementValue('temperature'), 0.8);
  const cfgWeight = safeParseFloat(safeGetElementValue('cfg-weight'), 0.5);
